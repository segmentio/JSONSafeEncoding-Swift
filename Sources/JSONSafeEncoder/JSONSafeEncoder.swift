...
/// `JSONSafeEncoder` facilitates the encoding of `Encodable` values into JSON.
open class JSONSafeEncoder {
    ...
    func encodeAsJSONValue<T: Encodable>(_ value: T) throws -> JSONValue {
        let encoder = JSONSafeEncoderImpl(options: self.options, codingPath: [])
...
// MARK: - _JSONSafeEncoder

private enum JSONFuture {
    case value(JSONValue)
    case encoder(JSONSafeEncoderImpl)
    ...
    class RefArray {
        ...
        @inline(__always) func append(_ encoder: JSONSafeEncoderImpl) {
    ...
    class RefObject {
        ...
        @inline(__always) func set(_ encoder: JSONSafeEncoderImpl, for key: String) {
            switch self.dict[key] {
            case .encoder:
                preconditionFailure("For key \"\(key)\" an encoder has already been created.")
            case .nestedObject:
...
private class JSONSafeEncoderImpl {
    let options: JSONSafeEncoder._Options
    ...
    init(options: JSONSafeEncoder._Options, codingPath: [CodingKey]) {
        self.options = options
        self.codingPath = codingPath
    }
}

extension JSONSafeEncoderImpl: Encoder {
...
extension JSONSafeEncoderImpl: _SpecialTreatmentEncoder {
    var impl: JSONSafeEncoderImpl {
        return self
    }
...
private protocol _SpecialTreatmentEncoder {
    ...
    var options: JSONSafeEncoder._Options { get }
    var impl: JSONSafeEncoderImpl { get }
}
...
extension _SpecialTreatmentEncoder {
    @inline(__always) fileprivate func wrapFloat<F: FloatingPoint & CustomStringConvertible>(_ float: F, for additionalKey: CodingKey?) throws -> JSONValue {
        guard !float.isNaN, !float.isInfinite else {
            /// TWILIO MODIFICATION - Replacement to expand handling options.
            switch self.options.nonConformingFloatEncodingStrategy {
            case .throw:
                break
            case .null:
                return .null
            case .zero:
                return .number("0")
            case .convertToString(let posInfString, let negInfString, let nanString):
                switch float {
                case F.infinity:
    ...
    func wrapObject(_ object: [String: Encodable], for additionalKey: CodingKey?) throws -> JSONValue {
        ...
        try object.forEach { (key, value) in
            ...
            let encoder = JSONSafeEncoderImpl(options: self.options, codingPath: elemCodingPath)
    ...
    fileprivate func getEncoder(for additionalKey: CodingKey?) -> JSONSafeEncoderImpl {
        if let additionalKey = additionalKey {
            var newCodingPath = self.codingPath
            newCodingPath.append(additionalKey)
            return JSONSafeEncoderImpl(options: self.options, codingPath: newCodingPath)
        }
...
private struct JSONKeyedEncodingContainer<K: CodingKey>: KeyedEncodingContainerProtocol, _SpecialTreatmentEncoder {
    ...
    let impl: JSONSafeEncoderImpl
    let object: JSONFuture.RefObject
    let codingPath: [CodingKey]

    private var firstValueWritten: Bool = false
    fileprivate var options: JSONSafeEncoder._Options {
        return self.impl.options
    }

    init(impl: JSONSafeEncoderImpl, codingPath: [CodingKey]) {
        self.impl = impl
    ...
    init(impl: JSONSafeEncoderImpl, object: JSONFuture.RefObject, codingPath: [CodingKey]) {
        self.impl = impl
        self.object = object
        self.codingPath = codingPath
    }
    ...
    private func _converted(_ key: Key) -> CodingKey {
        ...
        case .useDefaultKeys:
            return key
        case .convertToSnakeCase:
            let newKeyString = JSONSafeEncoder.KeyEncodingStrategy._convertToSnakeCase(key.stringValue)
...
private struct JSONUnkeyedEncodingContainer: UnkeyedEncodingContainer, _SpecialTreatmentEncoder {
    ...
    // used for nested containers
    init(impl: JSONSafeEncoderImpl, array: JSONFuture.RefArray, codingPath: [CodingKey]) {
        self.impl = impl
        self.array = array
        self.codingPath = codingPath
    }
...
extension EncodingError {
    ...
    fileprivate static func _invalidFloatingPointValue<T: FloatingPoint>(_ value: T, at codingPath: [CodingKey]) -> EncodingError {
        ...
        let debugDescription = "Unable to encode \(valueDescription) directly in JSON. Use JSONSafeEncoder.NonConformingFloatEncodingStrategy.convertToString to specify how the value should be encoded."
